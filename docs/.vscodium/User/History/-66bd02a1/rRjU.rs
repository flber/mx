...
fn run_plugins(config: &Config, path_str: &str, contents: &String) -> std::io::Result<String> {
  ...
  for (i, chr) in contents.char_indices() {
    if chr == '{' {
      ...
      for (j, chr) in slice(&contents, i..len(&contents)).char_indices() {
        // if tag.is_empty() && chr != '{' {
        //   println!("tag failed: {}", tag);
        //   tag = String::new();
        //   break;
        // }
        if !tag.is_empty() && chr != '}' {
        ...
        if !tag.is_empty() && chr == '}' {
          println!("hit close; next two: {}", contents.char_indices().nect();
          if slice(&contents, i..i + 1) != "}}" {
            println!("tag failed: {}", tag);
            tag = String::new();
            break;
          }
