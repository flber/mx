  ...
  pub fn parse_granite(s: &String, mut debug: bool) -> String {
    ...
    for (i, char) in s.char_indices() {
      ...
      match char {
        ...
        '[' => {
          if in_content && blacklist.contains(&elems[0]) {
            in_content = false;
          }
        }
        '|' => {
        ...
        if !in_content {
          match char {
            ...
            '[' => {
              ...
              let new_elem_str: String = new_elem.into_iter().collect();
              elems.push(new_elem_str);
            }
            ':' => output.push('='),
            ' ' => {
              if slice(s, i - 1..i) == ":" {
            ...
            ']' => {
              output.push_str("</");
              output.push_str(&elems.pop().unwrap_or("</>"));
              output.push('>');
            }
            _ => output.push(char),
          }
